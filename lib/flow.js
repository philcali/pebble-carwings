...
(function() {
  var updateCard = new UI.Card({
    ...
    , aboutCard = new UI.Card({
        title: 'Carwings',
        subtitle: 'Philip Cali',
        body: [
          'UI provided by PebbleJS.',
          'Telemetrics provided by CARWINGS.',
          'Charging station location provided by Open Charge Map.'
        ].join('\n')
      })
  ...
  function Flow(config, data) {
    ...
    this.menu.on('about', aboutCard.show);
  ...
  Flow.prototype.showInformation = function() {
    var body = [
      'Plugin State: ' + this.data.owner.vehicle.battery.pluginState,
      'Charging Status: ' + this.data.owner.vehicle.battery.charging,
      'Last Check: ' + this.data.owner.vehicle.battery.lastCheck,
    ];
    if (this.data.vehicle.battery.chargingTimes) {
      for (var time in this.data.vehicle.battery.chargingTimes) {
        var toFull = this.data.vehicle.battery.chargingTimes[time];
        body.push(['Time', 'on', time + ':', toFull.hours + 'h', toFull.minutes].join(' '));
      }
    }
