...
(function() {
  ...
  function Flow(config, data) {
    ...
    this.data = data;
    if (data.vehicle) {
      data.vehicle.lastCheck = new Date(data.vehicle.lastCheck);
    }
  ...
  Flow.prototype.launch = function(e) {
    if (!this.isConfigured()) {
      ...
      if (e && e.wakeup) {
        ...
        this.display.update(this.data.vehicle).show();
      }
    } else {
      this.login();
    }
  ...
  Flow.prototype.toggleAc = function(direction) {
    return (function() {
      if (this.config.acToggle != 'ac' + direction) {
        ...
        this.display.update(this.data.vehicle);
        Vibe.vibrate('short');
      }
    }).bind(this);
  };
  Flow.prototype.clickUpdate = function() {
    var now = Date.now()
      , check = this.data.vehicle.battery.lastCheck.getTime();
